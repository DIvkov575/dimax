...
mod tests {
    ...
    fn foreground_info_reports_the_running_shell_command() {
        ...
        let pane = ServerPane::spawn(
            id,
            None,
            // `exec` makes the pty's foreground *group leader* become cat
            // itself on every POSIX sh: some shells (notably dash on
            // Linux) fork the command instead of exec-ing it, leaving
            // the leader as `sh` forever, which `foreground_info`
            // reports.
            Some("exec cat".to_string()),
            // Explicit cwd: unset cwd makes portable-pty chdir($HOME),
            // which other tests are concurrently faking/deleting.
            Some(std::env::temp_dir().to_string_lossy().into_owned()),
            Size { rows: 24, cols: 80 },
            tx,
            "test-short-id".to_string(),
