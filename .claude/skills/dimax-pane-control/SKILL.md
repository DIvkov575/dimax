...
description: Drive dimax server-panes (persistent background shells/processes) directly via the `dimax` CLI over Bash -- spawn a pane, type commands into it, read back what it printed, rename/list/kill it. Use when the user asks to run something in a "dimax pane", keep a long-running process alive across turns, inspect what's happening in an existing dimax pane, or otherwise control dimax from the command line rather than through its interactive TUI.
---

# dimax pane control

`dimax` is a tmux-style terminal multiplexer with a daemon/client architecture. Its `server` subcommand gives full scripted control over **server-panes** — persistent PTY-backed processes the daemon keeps running — with no need to open the TUI (`dimax attach`), create a workspace, or bind a client-pane. Every command below auto-starts the daemon on first use if it isn't already running.

This skill covers the `dimax server ...` surface only: spawn, list, read, send, rename, kill. That is everything needed to run a process in the background, feed it input, and inspect its output — full control per the pane's lifecycle.
...
| Command | Effect |
|---|---|
| `dimax server spawn <name> [--cmd "<command>"]` | Start a new server-pane named `<name>`, running `<command>` (default: the user's `$SHELL` as a login shell). Prints the new pane's id. |
...
| `dimax server ls` | List every server-pane: `id  name  status  rowsxcols  process  cwd`, tab-separated, one per line. `status` is `running` or `dead`. |
...
- **Clean up panes you spawn** with `dimax server kill <name>` once you're done, unless the user asked for the pane to be left running (e.g. a dev server they want to keep using after you're done). Check `dimax server ls` if you're unsure what's still around from earlier in the session.
- **Check before spawning a duplicate.** If you might already have a pane for this purpose (e.g. from earlier in the same task), `dimax server ls` first rather than spawning a second one blindly — `spawn` will error anyway if you reuse a name that's still alive.
...
- **This is not the TUI.** `dimax attach` opens an interactive multi-pane terminal UI for a human; this skill's commands work headlessly against the same daemon and don't require (or interact with) any attached TUI session, workspace, or client-pane. A pane spawned here shows up in `dimax server ls` and can also be *bound into* a TUI workspace later (via `dimax client spawn --bind <target>` or the attach menu) if the user wants to watch it interactively — but that's outside this skill's scope.
